...
public final class EnderCloudVelocityPlugin implements EnderCloudVelocityApi {
    ...
    private final Map<String, ServerSnapshot> snapshots =
    ...
    public void onInitialize(ProxyInitializeEvent ignored) {
        ...
        String redisUrl = environment(
            "ENDERCLOUD_REDIS_URL",
        ...
        subscription
            ...
            .thenCompose(ignoredResult -> reloadSnapshot())
            .whenComplete((ignoredResult, error) -> {
                if (error != null) {
                    logger.error(
                        "Unable to initialize the EnderCloud registry",
                        error
                    );
                } else {
                    ready.set(true);
                    logger.info("EnderCloud Velocity bridge is ready");
                }
            });
    }
    ...
    public void onDisconnect(DisconnectEvent event) {
        ...
        orchestrator
            .networkDisconnected(event.getPlayer().getUniqueId())
            .exceptionally(error -> {
                logger.warn(
    ...
    private CompletableFuture<Void> reloadSnapshot() {
        return orchestrator.getServers().thenAccept(servers -> {
            ...
            servers.forEach(server ->
                snapshots.put(server.instanceId(), server)
            );
    ...
    private void handleRedisMessage(String channel, String message) {
        try {
            ...
            switch (envelope.type()) {
                ...
                case "TRANSFER_PLAYERS" -> transferPlayers(envelope.payload());
            ...
            logger.error(
                "Invalid EnderCloud Redis message on {}",
                channel,
                exception
            );
            reloadSnapshot().exceptionally(error -> {
                logger.error(
                    "Snapshot reload after an invalid event failed",
                    error
                );
                return null;
            });
    ...
    private void register(ServerSnapshot snapshot) {
        if (
            snapshot.endpoint() == null || snapshot.endpoint().isBlank()
        ) return;
        ...
        proxy.registerServer(
            new ServerInfo(name, parseEndpoint(snapshot.endpoint()))
        );
    }
    ...
    private void unregister(String instanceId) {
        ...
        proxy
            .getServer(serverName(snapshot))
            .ifPresent(server ->
                proxy.unregisterServer(server.getServerInfo())
            );
    }

    private void transferPlayers(JsonNode payload) {
        ...
        Optional<RegisteredServer> target = proxy.getServer(
            serverName(snapshot)
        );
        if (target.isEmpty()) {
            reloadSnapshot();
    ...
    static InetSocketAddress parseEndpoint(String endpoint) {
        ...
        if (separator <= 0 || separator == endpoint.length() - 1) {
            throw new IllegalArgumentException(
                "Invalid Minecraft endpoint: " + endpoint
            );
        }
