...
description: Environment files, ownership boundaries, validation, and project-wide variables.
---
...
are in [Orchestrator](/docs/configure/orchestrator) and [Agent](/docs/configure/agent).
...
See [the orchestrator environment table](/docs/configure/orchestrator#environment-variables) for requirements,
...
See [the agent environment table](/docs/configure/agent#environment-variables) for requirements, defaults, and
path semantics.
...
[the timeout reference](/docs/configure/timeouts).
...
the fields documented in [Server group configuration](/docs/configure/groups).
