...
const lifecycle = [
  {
    ...
    description: "Keep game servers warm so a full queue does not have to wait for a container to boot.",
  },
  {
    id: "02",
    name: "Parties stay together",
    description: "Reserve one server for the whole group instead of splitting friends across matches.",
  },
  {
    id: "03",
    name: "Players move automatically",
    description: "Send matched players through Velocity and track the session until the game ends.",
  },
  {
    id: "04",
    name: "Idle capacity disappears",
    description: "Close new joins, finish the match, then remove the server and recover its resources.",
  },
];
      ...
      <main>
        <section className="site-shell border-b border-border px-5 pt-20 pb-0 lg:px-8 lg:pt-28">
          <div className="grid gap-12 lg:grid-cols-[minmax(0,1.45fr)_minmax(260px,0.55fr)] lg:gap-20">
            <div className="max-w-5xl">
              <Reveal>
                <Badge variant="outline" className="rounded-sm border-primary/70 font-mono text-primary">
              ...
              <Reveal delay={0.08}>
                <h1 className="mt-7 max-w-5xl text-5xl leading-[0.96] font-semibold tracking-[-0.055em] text-balance sm:text-7xl lg:text-[6.25rem]">
                  Stop running Minecraft servers <span className="light-copy text-primary">by hand.</span>
                </h1>
              </Reveal>
              <Reveal delay={0.16}>
                <p className="mt-8 max-w-3xl text-lg leading-8 text-muted-foreground sm:text-xl">
                  EnderCloud keeps game servers ready before queues fill, starts more when demand rises, keeps parties together, and removes idle capacity when matches end.
                </p>
              </Reveal>
              <Reveal delay={0.24}>
                <div className="mt-9 flex flex-wrap items-center gap-3">
                  <Link className={cn(buttonVariants({ size: "lg" }), "light-action h-11 px-5 font-semibold")} href="/docs">
                    Read the docs
            ...
            <Reveal delay={0.18} direction="left">
              <aside className="border-t border-border pt-5 lg:mt-10">
                <p className="technical-label">WHAT ENDERCLOUD CHANGES</p>
                <dl className="mt-5 grid gap-0 font-mono text-xs">
                  <div className="flex justify-between border-b border-border py-3">
                    <dt className="text-muted-foreground">Waiting for boot</dt>
                    <dd>Warm capacity</dd>
                  </div>
                  <div className="flex justify-between border-b border-border py-3">
                    <dt className="text-muted-foreground">Split groups</dt>
                    <dd>Whole parties</dd>
                  </div>
                  <div className="flex justify-between border-b border-border py-3">
                    <dt className="text-muted-foreground">Idle servers</dt>
                    <dd>Automatic drain</dd>
                  </div>
                  <div className="flex justify-between py-3">
                    <dt className="text-muted-foreground">Host juggling</dt>
                    <dd className="inline-flex items-center gap-2"><span className="light-signal size-1.5 bg-signal" />Automatic placement</dd>
                  </div>
                </dl>
              </aside>
            </Reveal>
        ...
        <section id="product" className="site-shell border-b border-border">
          <div className="grid border-b border-border lg:grid-cols-[0.7fr_1.3fr]">
            ...
            <Reveal className="grid sm:grid-cols-2" delay={0.1} direction="left">
              {lifecycle.map((step, index) => (
                ...
                >
                  ...
                  <h3 className="mt-12 text-2xl font-semibold tracking-tight">{step.name}</h3>
                  <p className="mt-3 max-w-xs leading-7 text-muted-foreground">{step.description}</p>
                </article>
              ))}
            </Reveal>
          </div>
          ...
          <div className="px-5 py-16 lg:px-8 lg:py-24">
            <Reveal className="max-w-3xl">
              <p className="technical-label">FEATURES BUILT AROUND THE REAL WORK</p>
            ...
            <Reveal className="mt-12 grid border-t border-l border-border md:grid-cols-2 lg:grid-cols-3" delay={0.1}>
              {features.map(({ icon: Icon, code, title, description }) => (
                <article className="min-h-72 border-r border-b border-border bg-background p-6 lg:p-8" key={title}>
        ...
        <section id="solutions" className="site-shell border-b border-border px-5 py-16 lg:px-8 lg:py-24">
          <div className="grid gap-12 lg:grid-cols-[0.75fr_1.25fr] lg:gap-20">
            <Reveal>
              <p className="technical-label">SOLUTION / MULTI-HOST OPERATIONS</p>
              <h2 className="mt-5 text-4xl font-semibold tracking-[-0.035em] sm:text-5xl">
              ...
              <ol className="mt-10 border-t border-border">
                {solutions.map((solution, index) => (
                  <li className="flex items-center gap-5 border-b border-border py-4" key={solution}>
                    <span className="font-mono text-xs text-primary">0{index + 1}</span>
                    <span className="text-base font-medium">{solution}</span>
                    {index < solutions.length - 1 ? <ArrowRight aria-hidden="true" className="ml-auto size-4 text-muted-foreground" /> : <span className="light-signal ml-auto size-2 bg-signal" />}
                  </li>
                ))}
              </ol>
              <Link className={cn(buttonVariants({ variant: "outline" }), "mt-8")} href="/docs/configure/multi-host">
        ...
        <section className="site-shell border-b border-border">
          <div className="grid border-b border-border lg:grid-cols-2">
            <Reveal className="border-b border-border px-5 py-12 lg:border-r lg:border-b-0 lg:px-8 lg:py-16">
              <p className="technical-label">SOLUTION / CAPACITY AND MATCHMAKING</p>
            ...
            <Reveal className="px-5 py-12 lg:px-8 lg:py-16" delay={0.1} direction="left">
        ...
        <section className="site-shell border-b border-border px-5 py-16 lg:px-8 lg:py-24">
          <div className="grid gap-12 lg:grid-cols-[0.65fr_1.35fr] lg:gap-20">
            ...
            <Reveal className="grid border-t border-l border-border sm:grid-cols-3" delay={0.1} direction="left">
              {[
                { label: "Getting started", number: "01", href: "/docs/getting-started/overview", text: "Install the stack and automate your first server group." },
                { label: "Configure and operate", number: "02", href: "/docs/configure/groups", text: "Define groups, variants, hosts, and deadlines." },
                { label: "Integrate and develop", number: "03", href: "/docs/develop/minigame-integration", text: "Connect plugins and work across the repository." },
              ].map((item) => (
                <Link className="group flex min-h-64 flex-col border-r border-b border-border p-6 hover:bg-muted/50" href={item.href} key={item.label}>
                  ...
                  <ArrowRight aria-hidden="true" className="mt-auto size-4 transition-transform group-hover:translate-x-1" />
                </Link>
              ))}
            </Reveal>
          </div>
          ...
          <Reveal className="flex flex-col items-start justify-between gap-8 sm:flex-row sm:items-end">
            <div>
              <p className="technical-label">READY TO REMOVE THE MANUAL WORK?</p>
            ...
            <div className="flex shrink-0 flex-wrap gap-3">
              <Link className={cn(buttonVariants({ size: "lg" }), "light-action h-11 px-5")} href="/docs">
                Open docs
                <ArrowRight aria-hidden="true" data-icon="inline-end" />
              </Link>
              <a className={cn(buttonVariants({ variant: "outline", size: "lg" }), "h-11 px-5")} href={site.github} rel="noreferrer" target="_blank">
                GitHub
                <ExternalLink aria-hidden="true" data-icon="inline-end" />
              </a>
            </div>
